...
namespace ChartApp.Actors
{
    ...
    public class PerformanceCounterCoordinatorActor : ReceiveActor
    {
        ...
        /// we want to monitor.
        /// </summary>
        private static readonly Dictionary<CounterType,
            Func<PerformanceCounter>> CounterGenerators = new Dictionary<CounterType, Func<PerformanceCounter>>()
            {
                {CounterType.Cpu,
        ...
        /// <summary>
        /// Methods for crating new <see cref="Series"/> with distinct colors and names
        /// corresponding to each <see cref="PerformanceCounter"/>
        /// </summary>
        private static readonly Dictionary<CounterType,
            Func<Series>> CounterSeries = new Dictionary<CounterType, Func<Series>>()
            {
                {CounterType.Cpu,
        ...
        { 
            ...
            Receive<Watch>(watch =>
            {
                if (!_counterActors.ContainsKey(watch.Counter))
                {
                    ...
                    // one doesn's exist already
                    var counterActor = Context.ActorOf(
                        Props.Create(() => new PerformanceCounterActor(watch.Counter.ToString(), 
